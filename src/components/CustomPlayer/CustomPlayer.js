...
function CustomPlayer({ username, className, style, onReady, onError }) {
  ...
  const fetchHlsToken = useCallback(async (streamIdToFetch) => {
    try {
      const response = await fetch(`https://www.vimm.tv/api/hls/token/${streamIdToFetch}`, {
        method: 'POST',
        headers: {
  ...
  const refreshHlsToken = useCallback(async (oldToken) => {
    try {
      const response = await fetch('https://www.vimm.tv/api/hls/refresh-token', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  useEffect(() => {
    ...
    const fetchStreamUrl = async () => {
      try {
        ...
        const m3u8Url = `https://www.vimm.tv/live/${data.streamId}/master.m3u8`;
  ...
  }, [username, onError, fetchHlsToken]);
